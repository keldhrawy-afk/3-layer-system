...
export const SnapshotVaultTab: React.FC<SnapshotVaultTabProps> = ({ payload, auditResult }) => {
  ...
  const handleTakeManualSnapshot = () => {
    ...
    const newSnapshot: WeeklySnapshot = {
      ...
      spent: payload.ad_platforms[0]?.spend || 48500,
      impressions: payload.ad_platforms[0]?.impressions || 450000,
      clicks: payload.ad_platforms[0]?.clicks || 9500,
      raw_orders: payload.backend_sheet?.raw_orders || 233,
      confirmed_orders: payload.backend_sheet?.confirmed_orders || 121,
      delivered_orders: payload.backend_sheet?.delivered_orders || 98,
      cancelled_orders: payload.backend_sheet?.cancelled_fake_orders || 58,
      average_order_value: payload.backend_sheet?.average_order_value || 1200,
      cogs_per_order: payload.backend_sheet?.cogs_per_order || 350,
      blended_cpa: auditResult?.financial_economics?.true_cpa || 400.82,
      confirmed_revenue: (payload.backend_sheet?.confirmed_orders || 121) * (payload.backend_sheet?.average_order_value || 1200),
      net_contribution_margin: auditResult?.financial_economics?.net_profit || 54350,
      contribution_margin_pct: auditResult?.financial_economics?.contribution_margin_pct || 37.4,
      roas: auditResult?.financial_economics?.true_roas || 2.99,
      cvr: 2.45,
      decision_applied: {
        action_type: 'SCALE_BUDGET_20',
